...
function Trains() {
  ...
  const loadTrains = async () => {
    try {
      const trains = await axios.get("http://localhost:8082/train/getAll");
  ...
  const updateTrain = async (train) => {
    ...
    const updatedTrain = axios.put("http://localhost:8082/train/update/"+trainForEdit.id,train,{
      mode: "no-cors",
      headers: {
  ...
  const addTrain = (train) => {
    ...
    const newTrain = axios.post("http://localhost:8082/train/add",train)
  ...
  const deleteTrain = (train) => {
    ...
    const deletedTrain = axios.delete("http://localhost:8082/train/delete/"+train.id,{
      mode: "no-cors",
      headers: {
        "Access-Control-Allow-Origin": "*",
        "Content-Type": "application/json",
