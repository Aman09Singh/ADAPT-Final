...
function Stations() {
  ...
  const loadStations = async () => {
    try {
      const stations = await axios.get("http://localhost:8082/train/station/getAll", {
        mode: "no-cors",
        headers: {
  ...
  const updateStation = async (station) => {
    ...
    if(window.confirm("Do you really want to edit the station: " + station.name)){

      console.log(station.name + " " + station.number + " " );
    console.log(JSON.stringify(station) + "data");
    const updatedStation = axios.put("http://localhost:8082/train/station/update/"+stationForEdit.id,station,{
      mode: "no-cors",
      headers: {
        "Access-Control-Allow-Origin": "*",
        "Content-Type": "application/json",
  ...
  const addStation = (station) => {
    ...
    const newStation = axios.post("http://localhost:8082/train/station/add",station,{
      mode: "no-cors",
      headers: {
        "Access-Control-Allow-Origin": "*",
        "Content-Type": "application/json",
  ...
  const deleteStation = (station) => {
    ...
    if(window.confirm("Do you really want to delete the " + station.name)){

      console.log(station + " delete");
    const deletedStation = axios.delete("http://localhost:8082/train/station/delete/"+station.id,{
      mode: "no-cors",
      headers: {
        "Access-Control-Allow-Origin": "*",
        "Content-Type": "application/json",
